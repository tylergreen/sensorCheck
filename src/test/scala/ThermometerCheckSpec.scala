...
class ThermometerCheckSpec extends FlatSpec {
  ...
  "A ThermometerCheck" should "be 'ultra precise' if the mean of the readings is within 0.5 degrees of the known temperature and the std deviation is < 3" in {
  ...
  it should "be 'very precise' if the mean is within 0.5 and the std deviation is < 5" in {
    assertResult("VeryPrecise"){
      ThermometerCheck.classify(70.0, Array(70, 66, 74))
    }
  }

  it should "be pecise otherwise" in {
